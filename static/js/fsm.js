...
function saveBackup() {
	...
	// generate output json to copy fsm
	myStr = JSON.stringify(JSON.parse(localStorage['fsm']), null, 4);
	myStr = myStr.replace(/\"/g, '&quot;');
	myStr = '«canvas json="' + myStr;
	myW = document.getElementById("mywidth").value;
	myH = document.getElementById("myheight").value;
	myStr = myStr + '" width="' + myW + '" height="' + myH + '"»«/canvas»';
	document.getElementById("myOut").value = myStr;
}
...
function myImport() {
	...
	links = [];
 	...
 	myStr = document.getElementById("myIn").value;	
	
	myArr = myStr.split("\n");
	
	var lastLine = myArr[myArr.length-1];
	var firstLine = myArr[0];
	
	var arz = lastLine.split("\"")[2];
	var ertefa = lastLine.split("\"")[4];
	
	document.getElementById("mywidth").value = arz;
	document.getElementById("myheight").value = ertefa;
