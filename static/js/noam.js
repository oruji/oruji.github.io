  ...
  noam.re = (function() {
    ...
    var tree = (function() {
      ...
      */
        ...
        if (tree.tag === tags.ALT && tree.choices.length >=2) {
          ...
          for (var i=0; i<tree.choices.length-1; i++) {
            if (tree.choices[i].tag === tags.SEQ && tree.choices[i].elements.length >= 2) {
              ...
              for (var j=i+1; j<tree.choices.length; j++) {
                if (tree.choices[j].tag === tags.SEQ && tree.choices[j].elements.length >= 2) {
                  // if last element of choices "ab+cb" are equal <- added by @oruji
                  if (noam.util.areEquivalent(tree.choices[j].elements[tree.choices[j].elements.length-1],
                    ...
                    // select elements of first choice except last element <- added by @oruji
                    ...
                    // a+c | make a choice by a & c <- added by @oruji
                    var _alt = makeAlt([rest1, rest2]);
                    // (a+c)b | sequence a+c maded with last common element <- added by @oruji
      ...
      _regex_simplification_patterns.push({ 'pattern' : "$+L => L, if L contains $", 'type' : 'structure', 'function' : _regex_simplify_22});
      _regex_simplification_patterns.push({ 'pattern' : "L1+L2 => L2, if L1 is subset of L2", 'type' : 'structure', 'function' : _regex_simplify_19});
      _regex_simplification_patterns.push({ 'pattern' : "(L1+L2)* => L2, if L1* is subset of L2*", 'type' : 'structure', 'function' : _regex_simplify_20});
      _regex_simplification_patterns.push({ 'pattern' : "L1*L2* => L2, if L1* is subset of L2*", 'type' : 'structure', 'function' : _regex_simplify_21});
      _regex_simplification_patterns.push({ 'pattern' : "(L1+$)(L2)* => (L2)* if L1 is subset of L2", 'type' : 'structure', 'function' : _regex_simplify_27});
